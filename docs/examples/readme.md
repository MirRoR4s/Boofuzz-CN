...
四个步骤中除了原语构造部分，其余都可以交给 boofuzz 框架来做，所以针对某个协议的模糊测试实质上要做的工作就是熟悉、了解协议以此构造出原语。
...
通过研究官方给出的带有监视器的 FTP 协议模糊测试脚本，可以了解监视器的使用步骤和方法。不过可惜的是官方给出的脚本是依照旧的协议格式写的，后续的话笔者可能会转成新的。

阅读源码中的注释可以明白，此处的监视器其实就是一个启动 ftp 服务器的脚本。
